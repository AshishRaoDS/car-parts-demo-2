        ...
        if (addBtn) {
          addBtn.addEventListener('click', () => {
            ...
            fetch('api/cart/items', {
              ...
              .then((cart) => {
                window.analytics.track('added_to_cart', { productId: product.id });
                if (addConfirm) addConfirm.classList.add('detail-panel-confirm-visible');
              })
              .catch(() => {
                if (addConfirm) {
                  addConfirm.textContent = 'Unable to add to cart.';
  ...
  if (!cart.items.length) {
    main.innerHTML = `
      <div class="empty-state">
        ...
        <p class="empty-state-body">Nothing added yet. Browse the collection and add a piece you like &mdash; it will show up here.</p>
...
if (cartMain) {
  fetch('api/cart')
    ...
    .then((cart) => {
      renderCart(cart);
      window.analytics.track('cart_viewed', { itemCount: cart.items.length, total: cart.total });
    })
    .catch(() => {
